...
function WriteToId(id: string, text: string): void {
...
function F4(): void {
    let mostImprovedTeam: TeamData = GetMostImprovedTeam()[0];
...
function F5(): void {
    ...
    let teamName: string = countryInput.value;
    WriteToId("f5", `'${teamName}' <b>${IsTeamPartOfList(teamName) ? "megtalálható" : "nem található meg"}</b> a ranglistán.`);
}

function F6(): void {
    let data: object = GetTeamChangesStatistics();
    let table: HTMLTableElement = document.querySelector("#t6")!;
...
function GetTeamsAboveAverage(): TeamData[] {
    return Teams.filter(t => t.points > GetAveragePoints());
}

function GetMostImprovedTeam(): TeamData[] {
    return Teams.filter(t =>
        t.placeChange === Math.max(...Teams.filter(t =>
            t.placeChange > 0)
            .map(t => t.placeChange)));
}
 ...
 */
function IsTeamPartOfList(countryName: string): boolean {
    return Teams.some(t => t.name.toLowerCase() === countryName.toLowerCase());
}

function GetTeamChangesStatistics(): object {
    let result = {};
    for (const team of Teams) {
