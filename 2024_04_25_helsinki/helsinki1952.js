...
function CountAllContestants() {
    return places.reduce(SumContestants, 0);
}
function SumContestants(total, next) {
    return total + next.contestantCount;
}
function GetMedalsData(data) {
    const firstPlaces = GetPlaceDataForPlace(data, 1);
    const secondPlaces = GetPlaceDataForPlace(data, 2);
    const thirdPlaces = GetPlaceDataForPlace(data, 3);
    return {
...
function CreateStringDataArray() {
    let result = [];
...
function GetPlaceDataWithMostContestants() {
    return places.filter(value => value.contestantCount === Math.max(...places.map(value => value.contestantCount)))[0];
}
function SolveTasks() {
