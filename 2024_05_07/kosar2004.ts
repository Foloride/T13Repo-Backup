const eredmenyek: string[] = ["7up Joventut;Adecco Estudiantes;81;73;Palacio Mun. De Deportes De Badalona;2005-04-03",
...
interface StadionStatistics {
    stadionName: string;
    gamesCount: number;
}

function GetGameInfos(): GameInfo[] {
    return eredmenyek.map(value => {
        let rowData = value.split(";")
        return {
            homeTeamName: rowData[0],
            awayTeamName: rowData[1],
            homePoints: Number(rowData[2]),
            awayPoints: Number(rowData[3]),
            location: rowData[4],
            gameDate: rowData[5],
        }
    });
...
const Games: GameInfo[] = GetGameInfos();

function GetGameCountForTeam(teamName: string): GameCount {
    return {
        homeGamesCount: Games.filter(value => value.homeTeamName === teamName).length,
        awayGamesCount: Games.filter(value => value.awayTeamName === teamName).length
    }
}

function IsAnyGameDraw(): boolean {
    return Games.some(value => value.homePoints === value.awayPoints);
}

function GetTeamNameLike(association: string): string {
    return Games.find(value => value.homeTeamName.includes(association))?.homeTeamName ?? association;
}

function GetStatisticsForDate(date: string): GameInfo[] {
    return Games.filter(value => value.gameDate === date);
}

function GetStadionStatisticsWithMoreThan20Games(): StadionStatistics[] {
    let result = {};
    Games.forEach(value => {
        if (result[value.location] === undefined) {
    ...
    return Object.entries(result).map(value => {
        return {
            stadionName: value[0],
            gamesCount: Number(value[1])
        }
    }).filter(value => value.gamesCount > 20);
}

let gamesCountForRealMadrid: GameCount = GetGameCountForTeam("Real Madrid");
console.log(`3. feladat: Real Madrid: Hazai: ${gamesCountForRealMadrid.homeGamesCount}, Idegen: ${gamesCountForRealMadrid.awayGamesCount}`);
console.log(`4. feladat: Volt döntetlen? ${IsAnyGameDraw() ? "igen" : "nem"}`);
console.log(`5. feladat: barcelonai csapat neve: ${GetTeamNameLike("Barcelona")}`);
console.log(`6. feladat: `);
GetStatisticsForDate("2004-11-21").forEach(game => {
    console.log(`\t${game.homeTeamName}-${game.awayTeamName} (${game.homePoints}:${game.awayPoints})`);
});
console.log(`7. feladat: `);
GetStadionStatisticsWithMoreThan20Games().forEach(stadionStat => {
    console.log(`\t${stadionStat.stadionName}: ${stadionStat.gamesCount}`);
});